...
type CompiledAst =
    {
        ...
        compiledModule: Map<string, C.ModuleDeclaration> ref
        static_constructor: (ActionDecl * C.Statement list) list
    }
...
let writeFile
    ...
    let testPath =
        ...
        let file =
            Path
                .Combine(dir, Path.GetFileName(sourceFilePath))
                .Replace(".fs", ".fs.c")
    ...
    for i in (includes @ headers) do
        ...
        compiler.UpdateFile(
            sourceFilePath,
            FileCompilationResults.AddInclude,
            i
        )
    ...
    for item in compiledModule.Value do
        ...
        | C.StaticVar declarationInfo ->
            ...
            | C.ExprAssignment expr ->
                sb.AppendLine(
                    $"static {declarationInfo._type.ToTypeString()} {declarationInfo.name} = {Compiler.writeExpression expr};"
                )
                |> ignore
            | C.StatementAssignment statement ->
                sb.AppendLine(
                    $"static {declarationInfo._type.ToTypeString()} {declarationInfo.name};"
                )
                |> ignore
            ...
            | C.Default -> ()
        | C.Function functionInfo when
            functionInfo.id.Contains $"_anonymous_fn_"
            ->
            sb.AppendLine($"{Print.compiledFunctionSignature functionInfo};")
            |> ignore
    ...
    if static_constructor.Length > 0 then
        ...
        += (init_name, ((static_constructor |> List.map fst), init))
...
let transformFile (_com: Fable.Compiler) (file: File) =
    ...
    let rec loop decl =
        ...
        | ModuleDeclaration moduleDecl ->
            List.map loop moduleDecl.Members |> List.collect id
        | decl -> [ decl ]
    ...
    for decl in declarations do
        ...
        | MemberDeclaration memberDecl when memberDecl.Args.Length = 0 ->
            let value =
                ...
                | IdentExpr ident ->
                    C.StatementAssignment(
                        C.Block(transformMember context [] memberDecl.Body)
                    )
                | _ ->
                    if Query.isSimpleExpr memberDecl.Body then
                        C.ExprAssignment(
                            transformExpr context [] memberDecl.Body
                        )
                    else
                        C.StatementAssignment(
                            C.Block(transformMember context [] memberDecl.Body)
            ...
            State.addModuleStaticVar
                _com.CurrentFile
                (memberDecl.Name, memberDecl)

            compiledModule
            += (memberDecl.Name,
                C.StaticVar
                    {
                        _type = transformType [] memberDecl.Body.Type
            ...
            | MemberRef(ent, info) ->
                try
                    compiler.AddMember(
                        database.contents.GetMember(memberDecl.MemberRef)
                    )
                with ex ->
                    printfn $"{ex}"
                    ...
                    C.Function
                        {
                            id = name
                            args = []
                            return_type = transformType [] memberDecl.Body.Type
                            body =
            ...
            | Some m ->
                compiler.UpdateFile(
                    context.currentFile,
                    FileCompilationResults.AddMemberDeclaration,
                    (memberDecl, m, Unchecked.defaultof<_>)
                )
            | None ->
                for i in 1..10 do
                    print.printfn "========================================"
                ...
                print.printfn
                    $"Could not find member for ref: %A{memberDecl.MemberRef}"
        ...
        | ActionDeclaration actionDecl ->
            ...
            | _ ->
                let toCompile =
                    Function.gatherAnonymousFunctions context actionDecl.Body
                ...
                static_constructor <-
                    ...
                    @ [
                        (actionDecl,
                         (transformMember context [] actionDecl.Body))
                    ]
        ...
        | MemberDeclaration memberDecl ->
                    ...
                    try
                        ...
                        let attributes =
                            ...
                            |> Seq.map (fun attribute ->
                                attribute,
                                database.contents.TryGetEntity(
                                    attribute.Entity
                                )
                            )
                            |> Array.ofSeq
                        ...
                        let isImport =
                            _member.Attributes
                            |> Seq.exists (fun attr ->
                                attr.Entity.FullName = "System.Runtime.InteropServices.DllImportAttribute"
                            )
                        ...
                        if isImport then
                            log
                                $"================= dllImport {fullName} {memberDecl.Name} ===================="

                        for attribute in _member.Attributes do
                            log $"Attribute: %A{attribute.ConstructorArgs}\n"

                        Print.compiledMethodName (
                            memberDecl,
                            [],
                            _member.DeclaringEntity.Value
                        ),
                        isImport
                    with ex ->
                        print.printfn $"{ex}"
            ...
            if isImport then
                ...
                let args =
                    memberDecl.Args
                    |> List.map (fun arg -> arg.Name, transformType [] arg.Type)

                compiledModule
                += (compiledName, C.Extern(memberDecl.Name, args, return_type))
            ...
            else if Query.isGenericDecl memberDecl then
                match memberDecl.MemberRef with
                | MemberRef(declaringEntity, memberRefInfo) ->
                    try
                        ...
                        let accurateFullName =
                            if
                                m.GenericParameters.Length > 0
                                && not (declaringEntity.FullName.Contains "`")
                            then
                                declaringEntity.FullName
                                + $"`{m.GenericParameters.Length}"
                            else
                                declaringEntity.FullName

                        State.genericMethodDeclarations
                        := State.genericMethodDeclarations.contents.Add(
                            (accurateFullName, memberDecl.Name),
                            memberDecl
                        )

                        State.genericMethodDeclarations
                        := State.genericMethodDeclarations.contents.Add(
                            (declaringEntity.FullName, memberDecl.Name),
                            memberDecl
                        )
                    with ex ->
                        printfn $"{ex}"
                ...
                if displayExpressions then
                    let result =
                        Display.memberDeclaration database.contents memberDecl
                ...
                let toCompile =
                    Function.gatherAnonymousFunctions context memberDecl.Body
        ...
        | ClassDeclaration classDecl ->
            ...
            | Some ent ->
                ...
                if not isGeneric then
                    ...
                    let emitTypeAttr =
                        ent.Attributes
                        |> Seq.tryFind _.Entity.FullName.Contains("EmitType") // = Const.emitType)

                    if emitTypeAttr = None then
                        compiler.UpdateFile(
                            context.currentFile,
                            FileCompilationResults.AddClassDeclaration,
                            classDecl
                        )
                    ...
                    if not ent.IsFSharpUnion then
                        let (_includes, declarations) =
                            Function.Type.transformDeclaration
                                context
                                database.contents
                                []
                                classDecl
                        ...
                        for kv in declarations do
                            let emitTypeAttr =
                                ent.Attributes
                                |> Seq.tryFind
                                    _.Entity.FullName.Contains("EmitType") // = Const.emitType)
                    ...
                    | Some constructor ->
                        ...
                        match
                            database.contents.TryGetMember(
                        ...
                        State.genericMethodDeclarations
                        := State.genericMethodDeclarations.contents.Add(
                            (classDecl.Entity.FullName, constructor.Name),
                            constructor
                        )
                    //                    compiler.AddMember()
                    | None ->
        ...
        | _ -> printfn $"Transforming Declaration %A{decl}"

    print.printfn $"Compiled Module Size: {compiledModule.Value.Count}"

    writeFile _com.CurrentFile includes compiledModule static_constructor
    |> ignore
